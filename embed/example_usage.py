...
def main():
    ...
    try:
        ...
        log.info(
            f"Image embedding norm: {torch.norm(image_embeddings).item():.4f}"
        )
        ...
        batch_images = torch.stack([create_sample_image() for _ in range(3)])
        ...
        batch_fusion_embeddings = embed_model.embed_fused(
        ...
        # Text only
        text_forward = embed_model(
            text="Using the forward method with text only."
        )
        ...
        log.error(f"Model not found at path: {model_path}")
        log.info(
            "Please update the 'model_path' variable with the correct path to your LaViDa model."
        )
    except Exception as e:
        log.error(f"Error occurred: {e}")
        log.info(
            "Please make sure you have the correct model path and all dependencies installed."
        )
