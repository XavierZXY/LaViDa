...
device_map = "cuda:0"
conv_template = (
    "llada"  # Make sure you use correct chat template for different models
)
...
# FIM = ''
draft_answer = f"""
Sure, here is a poem about the image:
A<|mdm_mask|><|mdm_mask|><|mdm_mask|><|mdm_mask|><|mdm_mask|><|mdm_mask|><|mdm_mask|><|mdm_mask|><|mdm_mask|><|mdm_mask|><|mdm_mask|>{FIM}
...
image_tensor = [
    _image.to(dtype=torch.bfloat16, device=device) for _image in image_tensor
]
...
if True:
    draft_tokens = (
        tokenizer(draft_answer, return_tensors="pt")
        .to(input_ids.device)
        .input_ids
    )
    ...
    text_outputs = [
        text_output.lstrip("!").replace("<|endoftext|>", "")
