...
def get_num_transfer_tokens_sch(
    mask_index, steps, schedule=None, schedule_kwargs=None
):
    """
    In the reverse process, the interval [0, 1] is uniformly discretized into steps intervals.
    Furthermore, because LLaDA employs a linear noise schedule (as defined in Eq. (8)),
    ...
    for i in range(mask_num.size(0)):
        ...
        # print(sigmas_sample)
        # fix detal
        sigmas_sample = torch.clamp(
            sigmas_sample, 1, None
...
def generate(
    ...
    **kwargs,
    ...
    if prompt is None:
        ...
        prompt = torch.full((bsz, seq_len), 0, dtype=torch.long).to(
            model.device
        )
