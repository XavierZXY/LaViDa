...
from llava.model.language_model.llada.log_likelyhood import (
    get_log_likelihood as get_log_likelihood,
)
...
from .llada.generate import generate_embed as llada_generate_embed
...
class LlavaLladaModel(LlavaMetaModel, LLaDAModel):
    ...
    def __init__(
        ...
        LlavaMetaModel.__init__(
            self, pretrained_config, vision_kwargs=vision_kwargs, skip_init=True
        )
...
def sample_t(b, device, policy="uniform", policy_args=None):
    if policy == "uniform":
        ...
        u = (
            1
            - u
            - policy_args["mode_scale"]
            * (torch.cos(torch.pi * u / 2) ** 2 - 1 + u)
        )
        return u


def forward_process(
    bsz, seq_len, device, eps=1e-3, policy="uniform", policy_args=None
):
    b, l = bsz, seq_len
...
class LlavaLladaForMaskedDiffusion(LLaDAModelLM, LlavaMetaForCausalLM):
    ...
    def forward(
        ...
        attention_mask[raw_inputs_ids == eos_id] = (
            True  # no sequence attention mask per Sec B.1
        )
        ...
        if labels is not None:
            ...
            if self.prefix_lm:
                ...
                # print(prompt_len)
            noise_embeddings = self.get_model().transformer.wte(
                torch.tensor([mask_id]).to(raw_inputs_ids)
            )
            # noise_embeddings is 1, 4096
            bsz, seq_len = labels_mask.shape
            noise_embeddings = noise_embeddings.view(
                1, 1, -1
            )  # .repeat(bsz,seq_len,1)
            ...
            final_masked_indices = (
                masked_indices & labels_mask & (~infill_token_pos)
            )
            final_masked_indices_inv = (
                (~masked_indices) & labels_mask & (~infill_token_pos)
            )
            # breakpoint()
            # breakpoint()
            ...
            inputs_embeds_inv = torch.where(
                final_masked_indices_inv.view(bsz, seq_len, 1),
                noise_embeddings,
                inputs_embeds,
            )
            inputs_embeds = torch.where(
                final_masked_indices.view(bsz, seq_len, 1),
                noise_embeddings,
                inputs_embeds,
            )
            ...
            labels[
                labels == fim_id
            ] = -100  # kill infill token so we don't predict it
            ...
            final_masked_indices = torch.cat(
                [final_masked_indices, final_masked_indices_inv]
            )
